...
router.get("/", async (req: Request, res: Response) => {
    try {
        let results = await db.asyncPool('SELECT * FROM projects', []);
...
router.post("/insert", async (req: Request, res: Response) => {
    ...
    try {
        ...
        let results = await db.asyncPool('INSERT INTO projects (title,description, images,videos,files,tags,timelog,external,status) Values ($1,$2,$3,$4,$5,$6,$7,$8,$9)',
...
router.put("/update/:id", async (req: Request, res: Response) => {
    try {
        ...
        let { rows } = await db.asyncPool('UPDATE projects set title=$1, description=$2, images=$3, videos=$4, files=$5, tags=$6 ,timelog=$7 WHERE id =$8',
