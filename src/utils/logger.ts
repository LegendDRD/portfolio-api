...
export async function auditLog(userId:number,action:string) {
    ...
    const results = await db.asyncPool(`INSERT INTO audit (from_user, user_id, action) VALUES ('${user[0].name_first} ${user[0].name_last}','${userId}', '${action}')`, []);


}
export function emailCLog(src: string, info: string, cLogTrue: boolean = false) {

    let currentDate: Date | string = new Date(Date.now());
